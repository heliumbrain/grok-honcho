...
Ask questions one at a time in the conversation (sequential Q&A). Do not use Claude-only tools.
...
Before asking any questions, use the Honcho MCP `chat` tool to get a maximally thorough overview of what is already known about the user. Present a concise summary to the user, then tailor the interview to fill gaps or confirm uncertain areas.
...
After each answer, create exactly one concise conclusion and call Honcho MCP `create_conclusion`.
