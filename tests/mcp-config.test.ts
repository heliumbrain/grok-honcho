...
async function withMcp(
  config: unknown,
  fn: (client: Client, home: string) => Promise<void>,
): Promise<void> {
...
describe("MCP config", () => {
  ...
  test("set_config keeps session overrides when peerName changes", async () => {
    await withMcp(
      {
        apiKey: "test-key-not-real",
        peerName: "tester",
        sessions: { "/tmp/proj": "keep-me" },
        hosts: { grok: { enabled: true } },
      },
      async (client, home) => {
        const changed = await client.callTool({
          name: "set_config",
          arguments: { field: "peerName", value: "alice" },
        });
        expect(changed.isError).not.toBe(true);
        const payload = JSON.parse(toolText(changed as { content: Array<{ type: string; text?: string }> }));
        expect(payload.success).toBe(true);
        expect(payload.warnings?.[0]).toContain("session override");

        const onDisk = JSON.parse(readFileSync(join(home, ".honcho", "config.json"), "utf-8"));
        expect(onDisk.sessions).toEqual({ "/tmp/proj": "keep-me" });
      },
    );
  });
});
